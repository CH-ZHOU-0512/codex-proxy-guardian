...
> [!TIP]
> **v1.6.0 增加了事件监听。** Guardian 会监听 Codex 的日志、进程启动和更新结果；发现真实 WebSocket/流式失败时立即重新验证代理，不必等下一轮固定检查。自动更新每 60 分钟确认一次；即使计划任务失效，也会从安装目录安全兜底。监听只会验证和提示，不会因“正在重新连接”擅自关闭 Codex。
>
