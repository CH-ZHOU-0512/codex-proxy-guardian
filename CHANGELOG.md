...
- 新增重连信号去重、短时突发统计和安全归因状态。监听只负责重新验证和给出建议，不会因为日志出现“重连”就擅自关闭或重启 Codex。
- 自动更新增加 60 分钟心跳检查；即使更新计划任务丢失或动作不再指向本安装目录，也会使用安装目录内、带全局互斥锁的更新器兜底，并继续记录真实终态。
- Windows 更新请求优先使用系统自带 `curl.exe`（包括 HTTP/HTTPS 代理和直连回退），避开部分 PowerShell 网络栈对 GitHub 返回的误报 403；更新结果仍会写入 `update-status.json`。
- Codex 更新后继续重新解析 MSIX 清单并刷新兼容指纹；监听、版本变化和更新终态全部写入 Status/Doctor，便于用户和 Issue 直接判断问题在哪一层。
- macOS/Linux 自动更新检查间隔同步支持配置，默认 60 分钟；保留原有安全校验、回滚和不修改系统网络配置的边界。
...
- 操作改为“重启Codex”和“60分钟后再提醒我”。默认焦点、关闭按钮、Esc 与倒计时都选择稍后提醒；只有明确点击“重启Codex”才会继续原有的受控重启链路。
- 启用 Per-Monitor V2 DPI 感知与自动文字测量，避免高分屏或非 100% 缩放下的窗口、文字发虚或字形边缘被固定高度遮挡。品牌窗口无法加载时，回退为非系统模态、信息图标、默认“否”的温和系统提示。
- Settings 的流式连接入口与确认说明同步改为更自然的产品语言；新增主题色、安全默认值、DPI 与旧警告样式移除的回归断言。
